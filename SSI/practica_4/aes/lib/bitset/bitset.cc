...
Bitset::Bitset(int bitset, int size) {
	if (size == 0)
		_bitset = new dynamic_bitset<unsigned char>(AesFunctions::int2bin(bitset));
	else if (size > 0)
...
void Bitset::shiftSelf(const int n, const char MODE) {
	int times = n % size();
	switch (MODE) {
		case LEFT: {
			//Hay que cambiar cada bit por el anterior y el primero a 0
			for (int i = 0; i < times; i++) {
				(*_bitset)<<=1;
				//(*_bitset)[0] = ultimo;
			}
		...
		case RIGHT: {
			//Hay que cambiar cada bit por el siguiente y el ultimo a 0
			for (int i = 0; i < times; i++) {
				(*_bitset)>>=1;
				//(*_bitset)[size() -1] = primero;
			}
...
Bitset Bitset::shift(const int n, const char MODE) {
	Bitset aux = *this;
	aux.shiftSelf(n,MODE);
	return aux;
}
