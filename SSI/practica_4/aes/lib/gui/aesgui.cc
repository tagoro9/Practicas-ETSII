...
AesGui::AesGui(char* path) : _path(path) {
   ...
   boton->signal_clicked().connect(sigc::mem_fun(*this,&AesGui::on_cifrar_clicked));
   //Entradas de texto
   ...
   dat.entry = coded_e;
   key_e->set_text("2b7e151628aed2a6abf7158809cf4f3c");
   message_e->set_text("3243f6a8885a308d313198a2e0370734");
}

AesGui::~AesGui() {
}

Gtk::Window* AesGui::get_window() {
   return window;
}

string AesGui::get_working_path()
{
   char temp[100];
   string aux;
...
void AesGui::cb_child_watch( GPid  pid, gint  status) {
    /* Close pid */
    g_spawn_close_pid( pid );
}

gboolean AesGui::cb_out_watch( GIOChannel *channel, GIOCondition  cond,Data *out ) {
    gchar *string;
    gsize  size;
    if( cond == G_IO_HUP )
    {
...
gboolean AesGui::cb_err_watch( GIOChannel   *channel, GIOCondition  cond,Data *err)
{
    gchar *string;
    gsize  size;
